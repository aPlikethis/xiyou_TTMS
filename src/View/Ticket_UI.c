...
void Ticket_UI_MgtEntry(int schedule_id)
{
    ...
    do{
		...
		printf("           %-6s                 %.2d                %d/%.2d/%.2d            %.2d:%.2d:%.2d\n\n\n",
                need_play->name,need_schedule->id,
        ...
        { 
		    switch(choice)
            {
		        case 'a':
	        	case 'A':
		         	if (Ticke_Srv_GenBatch(schedule_id, need_schedule->play_id))
			        	printf("生成成功!\n");
			     else
				        printf("生成失败!\n");
			        break;
		        case 'd':
		        case 'D':
			        if(Ticket_Sry_DeleteBatch(schedule_id) && Ticker_Srv_GenBatch(schedule_id, need_schedule->play_id))
				        printf("重新生成成功");
			        else
				        printf("重新生成失败");
			        break;
